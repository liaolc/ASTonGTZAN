...
def validate(audio_model, val_loader, args, epoch):
    ...
    return stats, loss
def evaluate(audio_model, val_loader):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #batch_time = AverageMeter()
    if not isinstance(audio_model, nn.DataParallel):
        audio_model = nn.DataParallel(audio_model)
    ...
    with torch.no_grad():
        ...
        #   then, run prediction on all 3 independently
        #   then, check for majority: use dictionary with keys = class#, value = # seen
        #   
        # Labels = tensor of shape (batchsize * 2, num classes) with 1's in the column of the true class
        # Don't know why removing batchsize * 2 in run.py breaks it
        # so batches of 6 segments = 2 songs -- purposely not just splitting tensor in half
        ...
        for i, (audio_input, labels) in enumerate(val_loader):
            ...
            # if i % 3 != 0 or i == 0: 
            #     piece_labels.append(labels)
            #     if(len(piece_labels) > 1):
            #         assert piece_labels[i] == piece_labels[i-1]
            s1_audio_input = s1_audio_input.to(device)
            s2_audio_input = s2_audio_input.to(device)

            # compute output -- could be done w/o naive copy paste but its 5am and the sun is rising
            s1_audio_output = audio_model(s1_audio_input)
            s1_audio_output = torch.sigmoid(s1_audio_output)
